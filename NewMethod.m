...
if doTrain   
    ...
    pdrid = trainNetwork(patches_train, PDRID, options);
    save('pdrid','pdrid');
...
if docheck
    ...
    pred = predict(pdrid, im_test{1,1});
...
FilePath = 'dataset_single/testingset/';%qiang
% FilePath3 = 'reimage2/';
...
for i = 1:img_num
    ...
    while temp ~= 1
        ...
        if temp == 2
            options.delta = options.delta*0.8;
            options.max_iter = 15;
        else
            reCoef = reCoef*0.9;
        end

        [xsol, bp_y, t, temp] = PnP(pdrid, image, options, reCoef, i);
    end
    %[xsol, bp_y, t] = PnP(dncnn, image, options, reCoef);
    execuateTime = toc;
...
%     filename = image_name;
%     filename(end-3:end) = 'fits';               
%     write_filename = [FilePath3 filename];
%     fitswrite(xsol,write_filename);
